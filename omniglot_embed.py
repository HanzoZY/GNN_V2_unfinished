...
class OmniglotEmbedNetwork:
    def __init__(self, inputs, batch_size):
        ...
        # output : B x T x D
        self.input_placeholder, self.label_placeholder = inputs #data and label all in

        # self.input_placeholder=inputs[:-1] # this is used for the test behind
        print(np.shape(self.input_placeholder))
    ...
    def add_block(self, x, in_channel, out_channel):
        ...
        batch_normalized = tf.nn.batch_normalization(conv_output, batch_mean, batch_var, beta, gamma, self.epsilon) #batch normalization
...
def _OmniglotEmbed_test():
    with tf.Graph().as_default():
        ...
        model = OmniglotEmbedNetwork(out_data, 5)

        with sess.as_default():
            init = tf.global_variables_initializer()
            sess.run(init)
            # out_data_a=sess.run(out_data)

            output, = sess.run([model.output])
            print(np.shape(output))
            # print(output.shape)
            # print(np.shape(out_data_a[:-1]),np.shape(out_data_a[-1]))
